...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
    ...
    for (h=h0; h<=ctl->top; h++) {
        ...
        if (sht->col_inv == -1) { // no transparent color
            if ((sht->vx0 & 3) == 0 && (bx0 & 3) == 0 && (bx1 & 3) == 0) { // optimize 4 pixels at a time
                bx1 = (bx1 - bx0) / 4;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
    ...
    for (h=h0; h<=h1; h++) {
        ...
        if ((sht->vx0 & 3) == 0) { // optimize 4 pixels at a time
            i = (bx0 + 3) / 4;  // bx0 / 4 rounded up
            i1 = bx1 / 4;       // bx1 / 4 rounded down
            i1 = i1 - i;
            sid4 = sid | sid << 8 | sid << 16 | sid << 24;
            for (by=by0; by<by1; by++) {
                vy = sht->vy0 + by;
                for (bx=bx0; bx<bx1 && (bx&3)!=0; bx++) {   // 1 pixels at a time
                    vx = sht->vx0 + bx;
                ...
                for (i=0; i<i1; i++) {  // 4 pixels at a time
                    if (p[i] == sid4) {
                        q[i] = r[i];
                    } else {
                        bx2 = bx + i * 4;
                ...
                for (bx+=i1*4; bx<bx1; bx++) {  // 1 pixels at a time
                    vx = sht->vx0 + bx;
                    if (map[vy * ctl->xsize + vx] == sid) {
                        vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
                    }
